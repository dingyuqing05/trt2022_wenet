...
import wenet.transformer.quantization_util as qu
...
def main():
    ...
    if False and args.rank == 0:  # dingyuqing
    ...
    quant_start_epoch = configs.get('quant_start_epoch', 100) # dingyuqing
    qu.disable_quant(model)
    #assert(quant_start_epoch < num_epochs)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if True: # dingyuqing
            if quant_start_epoch == epoch:
                qu.enable_calib(model)
            if quant_start_epoch + 1 == epoch:
                qu.load_calib_amax(model)
