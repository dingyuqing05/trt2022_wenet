    ...
    real_weights_num_(5)
{
    ...
    for (size_t l = 0; l < num_layer_; l++) {
        encoder_layer_weights.push_back(new WenetEncoderLayerWeight<T>(l, head_num_, size_per_head, inter_size_, int8_mode_));
    }
...
void WenetEncoderWeight<T>::initialize()
{
    ...
    weights_size[4] = 1*5000*hidden_size;
...
void WenetEncoderWeight<T>::loadModel(std::string dir_path)
{
    ...
    weights_name.push_back(name_prefix + "embed.pe");

    for (size_t i = 0; i < weights_name.size(); ++i) {
        loadWeightFromBin<T>(
            weights_ptr[i], {(int)weights_size[i]}, dir_path + weights_name[i] + ".bin", model_file_type);
    }
