...
template<typename T>
void RelPositionAttentionLayer<T>::initialize()
{
    allocateBuffer();
}

template<typename T>
void RelPositionAttentionLayer<T>::forward(std::vector<fastertransformer::Tensor>* output_tensors,
                                           ...
                                           const RelPositionMultiHeadedAttentionWeight<T>* attention_weights)
{
    ...
    FT_CHECK(input_tensors->size() == 4 || input_tensors->size() == 5);
    // allocateBuffer();
    ...
    const T* pos_emb = (const T*)input_tensors->at(3).data;
    const T* relative_attention_bias = input_tensors->size() == 5 ? (const T*)input_tensors->at(4).data : nullptr;
    ...
    if (sparse_ && cublas_wrapper_->isUseSparse(1, n, m, k)) {
        ...
        if (is_batched_QKV_) {
            ...
            if (last_q_w != attention_weights->query_weight.kernel || last_k_w != attention_weights->key_weight.kernel
                || last_v_w != attention_weights->value_weight.kernel || last_q_in != from_tensor
                || last_k_in != from_tensor || last_v_in != from_tensor) {
                ...
                last_q_in = const_cast<T*>(from_tensor);
                last_k_in = const_cast<T*>(from_tensor);
                last_v_in = const_cast<T*>(from_tensor);
            }
    ...
    cublas_wrapper_->stridedBatchedGemm(CUBLAS_OP_T,
                                        ...
                                        */
                                        pos_emb,
                                        size_per_head_,
                                        input_tensors->at(3).shape[2] * size_per_head_,
                                        q_buf_bias_v_,
                                        size_per_head_,
                                        request_seq_len * size_per_head_,
                                        qp_buf_,
...
void RelPositionAttentionLayer<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        ...
        p_buf_ =
            (T*)allocator_->malloc(sizeof(T) * max_batch_size_ * max_seq_len_ * hidden_units_, false);  // (BS, N, H)
        p_buf_2_ =
            (T*)allocator_->malloc(sizeof(T) * max_batch_size_ * max_seq_len_ * hidden_units_, false);  // (B,N,H,S)
        q_buf_bias_v_ =
            (T*)allocator_->malloc(sizeof(T) * max_batch_size_ * max_seq_len_ * hidden_units_, false);  // (B,N,S,H)
        qp_buf_ = (T*)allocator_->malloc(sizeof(T) * max_batch_size_ * head_num_ * max_seq_len_ * max_seq_len_,
                                         false);  // (B,N,S,S)
...
void RelPositionAttentionLayer<T>::freeBuffer()
{
    ...
    if (is_allocate_buffer_) {
        ...
        allocator_->free(p_buf_);
        allocator_->free(p_buf_2_);
        allocator_->free(q_buf_bias_v_);
        allocator_->free(qp_buf_);
...
template<typename T>
bool RelPositionAttentionLayer<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
bool RelPositionAttentionLayer<T>::isValidSeqLen(size_t seq_len)
{
